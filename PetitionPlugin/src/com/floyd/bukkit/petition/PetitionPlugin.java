...
//    }
	...
	public void onEnable() {
        ...
        new PetitionPlayerListener(this);
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args ) {
        ...
        if (cmdname.equalsIgnoreCase("pe") || cmdname.equalsIgnoreCase("petition") || cmdname.equalsIgnoreCase("ticket")) {
        	if (player == null || player.hasPermission("petition")) {
	        	// Help
	        	if (args.length == 0) {
	        		performHelp(player);
	        		return true;
	        	...
	        	if (args.length >= 1) {
                    ...
                    if (args[0].equalsIgnoreCase("version")) {
                        sender.sendMessage(ChatColor.YELLOW + "This server is running [PetitionPlugin] " + "version " + this.getDescription().getVersion());
                        return true;
                    }
	        	}
	        	if (args.length >= 2) {
	        		// View
	        		if (args[0].equalsIgnoreCase("view") || args[0].equalsIgnoreCase("lesen") || args[0].equalsIgnoreCase("anzeigen")) {
	        			performView(player, args);
	        			return true;
	        		}
	        		// Assign
	        		if (args[0].equalsIgnoreCase("assign") || args[0].equalsIgnoreCase("nehmen") || args[0].equalsIgnoreCase("annehmen")) {
	        			performAssign(player, args);
	        			return true;
	        		}
	        		// Unassign
	        		if (args[0].equalsIgnoreCase("unassign") || args[0].equalsIgnoreCase("abgeben") || args[0].equalsIgnoreCase("freigeben")) {
	        			performUnassign(player, args);
	        			return true;
	        		}
	        		// Close
	        		if (args[0].equalsIgnoreCase("close") || args[0].equalsIgnoreCase("zu") || args[0].equalsIgnoreCase("schliessen")) {
	        			performClose(player, args);
	        			return true;
	        		}
	        		// Reopen
	        		if (args[0].equalsIgnoreCase("reopen") || args[0].equalsIgnoreCase("nochmal")) {
	        			performReopen(player, args);
	        			return true;
	        		}
	        		// Comment
	        		if (args[0].equalsIgnoreCase("comment") || args[0].equalsIgnoreCase("log") || args[0].equalsIgnoreCase("kommi")) {
	        			performComment(player, args);
	        			return true;
	        		}
	        		// Open
	        		if (args[0].equalsIgnoreCase("open") || args[0].equalsIgnoreCase("new") || args[0].equalsIgnoreCase("create") || args[0].equalsIgnoreCase("neu")) {
	        			performOpen(player, args);
	        			return true;
	        		}
	        		// Warp
	        		if (args[0].equalsIgnoreCase("warp") || args[0].equalsIgnoreCase("goto") || args[0].equalsIgnoreCase("tp")) {
	        			performWarp(player, args);
	        			return true;
	        		}
	        	}
        	} else {
       			logger.info("[Pe] Zugriff verweigert fuer " + player.getName());
        	}
        }
        return false;
    }
    ...
    private void performWarp(Player player, String[] args) {
    	...
    	String name = "(Console)";
    	if (player == null) {
    		respond(player, "[Pe] Das waere ein netter Trick.");
    		return;
    	} else {
       		name = player.getName();
    	}
    		...
    		if (petition.isValid() && (petition.isOpen() || moderator)) {
    			if (canWarpTo(player, petition)) {
					...
					if (player.teleport(petition.getLocation(getServer()))) {
	    				respond(player, "[Pe] §7Teleportieren zum Ort an dem das " + settings.get("single") + " erstellt wurde." );
						logger.info(name + " teleported to petition " + id);
					} else {
	    				respond(player, "[Pe] §7Teleport fehlgeschlagen." );
						logger.info(name + " teleport to petition " + id + " FAILED");
					}
    			} else {
    				logger.info("[Pe] Access to warp to #" + id + " denied for " + name);
    				respond(player, "§4[Pe] Zugriff verweigert.");
    			}
    		} else {
    			respond(player, "§4[Pe] Kein offenes " + settings.get("single").toLowerCase() + " #" + args[1] + " gefunden." );
    		}
		}
		finally {
		...
		try {
    		...
    		if (petition.isValid() && petition.isOpen()) {
    			if (petition.ownedBy(player) || moderator) {
	        		...
	        		// Notify
	        		notifyNamedPlayer(petition.Owner(), "[Pe] §7Dein " + settings.get("single") + " §6#" + id + "§7 wurde aktualisiert: " + message );
					notifyNamedPlayer(petition.Assignee(), "[Pe] §7" + settings.get("single") + " §6#" + id + "§7 wurde aktualisiert von " + name + ".");
					String[] except = { petition.Owner(), petition.Assignee() };
	    			notifyModerators("[Pe] §7" + settings.get("single") + " §6#" + id + "§7 Kommentar hinzugefuegt von " + name + ".", except);
	        		petition.Comment(player, message);
					logger.info(name + " commented petition " + id + ". " + message);
    			} else {
    				logger.info("[Pe] Zugriff auf Kommentar #" + id + " verweigert fuer " + name);
    			}
    		} else {
    			respond(player, "§4[Pe] Kein offenes " + settings.get("single") + " #" + args[1] + " gefunden." );
    		}
		}
		finally {
			releaseLock(id, player);
		...
		try {
    		...
    		if (petition.isValid() && petition.isOpen()) {
    			if (petition.ownedBy(player) || moderator) {
	        		...
	        		// Notify
	        		notifyNamedPlayer(petition.Owner(), "[Pe] §7Dein " + settings.get("single") + " §6#" + id + "§7 wurde geschlossen. " + message );
					notifyNamedPlayer(petition.Assignee(), "[Pe] §7" + settings.get("single") + " §6#" + id + "§7 wurde geschlossen von " + name + ".");
					String[] except = { petition.Owner(), petition.Assignee() };
		...
		try {
    		...
    		if (petition.isValid() && petition.isClosed()) {
    			if (moderator) {
	        		...
	        		// Notify
	        		notifyNamedPlayer(petition.Owner(), "[Pe] §7Dein " + settings.get("single") + " §6#" + id + "§7 wurde wieder geoeffnet. " + message );
					notifyNamedPlayer(petition.Assignee(), "[Pe] §7" + settings.get("single") + " §6#" + id + "§7 wurde wieder geoeffnet von " + name + ".");
					String[] except = { petition.Owner(), petition.Assignee() };
	        		notifyModerators("[Pe] §7" + settings.get("single") + " §6#" + id + "§7 wurde wieder geoeffnet. " + message, except );
    			...
    			respond(player, "§4[Pe] Kein geschlossenes " + settings.get("single") + " #" + args[1] + " gefunden." );
    		}
		}
		finally {
			releaseLock(id, player);
        ...
        try {
            ...
            if (petition.isValid() && petition.isOpen()) {
                ...
                if (Boolean.parseBoolean(settings.get("notify-owner-on-unassign"))) {
                    notifyNamedPlayer(petition.Owner(), "[Pe] §7Dein " + settings.get("single") + " §6#" + id + "§7 wurde wieder freigegeben.");
                }
                notifyNamedPlayer(petition.Assignee(), "[Pe] §7" + settings.get("single") + " §6#" + id + "§7 wurde dir von " + name + " genommen und freigegeben.");
                String[] except = { petition.Owner(), petition.Assignee() };
                notifyModerators("[Pe] §7" + settings.get("single") + " §6#" + id + "§7 wurde von " + name + " freigegeben.", except);
    ...
    private void performAssign(Player player, String[] args) {
        ...
        if (moderator == false) {
            ...
            respond(player, "§4[Pe] Nur Moderatoren koennen "+settings.get("plural") + " zuweisen.");
            return;
        }
        try {
            getLock(id, player);
            ...
            if (petition.isValid() && petition.isOpen()) {
                ...
                if (Boolean.parseBoolean(settings.get("notify-owner-on-assign"))) {
                    notifyNamedPlayer(petition.Owner(), "[Pe] §7Dein " + settings.get("single") + " §6#" + id + "§7 wurde an " + petition.Assignee() + " zugewiesen.");
                }
                notifyNamedPlayer(petition.Assignee(), "[Pe] §7" + settings.get("single") + " §6#" + id + "§7 wurde dir von " + name + " zugewiesen.");
                String[] except = { petition.Owner(), petition.Assignee() };
                notifyModerators("[Pe] §7" + settings.get("single") + " §6#" + id + "§7 wurde " + petition.Assignee() + " zugewiesen.", except);
    ...
    private void performList(Player player, String[] args) {
		...
		if (args.length >= 2) {
			for (Integer index = 1; index < args.length; index++) {
				if (args[index].equalsIgnoreCase("closed") || args[index].equalsIgnoreCase("zu")) {
					use_archive = true;
					continue;
				}
				if (args[index].equalsIgnoreCase("newest") || args[index].equalsIgnoreCase("neueste") || args[index].equalsIgnoreCase("neuestes")) {
					sort_reverse = true;
					continue;
				}
				if (args[index].equalsIgnoreCase("unassigned") || args[index].equalsIgnoreCase("freie")) {
    	...
    	respond(player, "[Pe] §7"+(use_archive?"Geschlossene":"Offene")+" " + settings.get("plural") + (pattern==null?"":" matching "+filter) + ": " + count + " (Zeige: " + showing + ")" );    	
    }
    ...
    private void performHelp(Player player) {
        ...
        respond(player, "[Pe] §7" + settings.get("single") + " Benutzung: (statt /pe kann auch /ticket verwendet werden!)");
    	respond(player, "[Pe] §7/" + cmd + " neu|open|create|new <Grund>");
    	respond(player, "[Pe] §7/" + cmd + " kommi|comment|log <#> <Kommentar>");
    	respond(player, "[Pe] §7/" + cmd + " zu|close <#> [<Kommentar>]");
    	respond(player, "[Pe] §7/" + cmd + " liste|list [online|offline|neuestes|newest|zu|closed|frei|unassigned] [<Anzahl>|<Muster>]");
    	respond(player, "[Pe] §7/" + cmd + " lesen|view <#>");
    	if (canWarpAtAll(player)) {
    		respond(player, "[Pe] §7/" + cmd + " tp|warp|goto <#>");
    	}
        if (moderator) {
        	respond(player, "[Pe] §7/" + cmd + " nehmen|annehmen|assign <#>");
        	respond(player, "[Pe] §7/" + cmd + " abgeben|freigeben|unassign <#>");
        	respond(player, "[Pe] §7/" + cmd + " nochmal|reopen <#> [<Kommentar>]");
        }
    }
